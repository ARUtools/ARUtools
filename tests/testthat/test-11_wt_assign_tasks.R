test_that("wt_assign_tasks()", {
  ...
  expect_warning(
    task_output_file_no_seed <- wt_assign_tasks(wt_task_template_in = task_template,
  ...
  set.seed(1231)
  expect_silent(
  task_output <- wt_assign_tasks(wt_task_template_in = task_template,
                                 wt_task_output_file = NULL,
                                 interp_hours = template_observers,
                                 interp_hours_column = hrs,
                                 random_seed = 65122
  ...
  expect_no_warning(
  task_output_file <- wt_assign_tasks(wt_task_template_in = tf_tasks,
                                 wt_task_output_file = NULL,
                                 interp_hours = tf,
                                 interp_hours_column = hrs,
                                 random_seed = 65122
  ...
  expect_error(task_output_file <- wt_assign_tasks(wt_task_template_in = tf_no_csv,
                                                   wt_task_output_file = NULL,
                                                   interp_hours = template_observers,
                                                   interp_hours_column = hrs,
                                                   random_seed = 65122
  ...
  expect_error(task_output_file <- wt_assign_tasks(wt_task_template_in = task_template,
                                                   wt_task_output_file = NULL,
  ...
  task_output_written <- wt_assign_tasks(wt_task_template_in = task_template,
                                 wt_task_output_file = tf_out,
                                 interp_hours = template_observers,
                                 interp_hours_column = hrs,
                                 random_seed = 65122
  )
  tf_out_in <- readr::read_csv(tf_out, col_types = glue::glue_collapse(rep("c", 13)),
                               na = character() )

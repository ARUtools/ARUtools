...
```{r}
in_tasks <- tempfile(fileext = ".csv")
task_template <- wildRtrax::wt_make_aru_tasks(example_clean |> 
                               mutate(
...
template_observers <- 
  "https://raw.githubusercontent.com/dhope/Shiny_select/main/data/InterpreterHours_Template.csv"
...
```{r}
task_output <- wt_assign_tasks(wt_task_template_in = in_tasks, 
                wt_task_output_file = NULL,#out_tasks,
                interp_hours_file = template_observers,
                interp_hours_column = hrs, 
                random_seed = 65416
                )

gt::gt(task_output$task_summary)

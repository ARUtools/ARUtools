...
However, ARU GPS logs are notoriously unreliable, so it is recommended that you
...
There are two options. The first is that each ARU unit was set up to record in 
...
The second option is that all ARUs were set up at home base before deployment.
...
specify it here. In this case, use `tz = "America/Toronto"`, or which ever time
